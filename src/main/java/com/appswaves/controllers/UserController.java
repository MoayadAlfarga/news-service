...
public class UserController {
    ...
    public ResponseEntity<AuthenticationResponse> registerUser(@RequestBody RegistrationUserDto registerUserDto) {
        return ResponseEntity.ok(usersService.registrationUser(registerUserDto));
    }
    @PostMapping("/login")
    public ResponseEntity<AuthenticationResponse> authenticationUser(@RequestBody LoginRequestDto loginRequestDto) {
